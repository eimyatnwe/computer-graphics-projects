...
class Tank(GameObject):
    ...
    def draw_tank_hull(self, width, height, depth):
        """Draw main tank body with beveled edges - WIREFRAME"""
        ...
        # Connect front to back (vertical edges)
        glBegin(GL_LINES)
        glVertex3f(-w, -h, d)
        glVertex3f(-w, -h, -d)
    ...
    def draw_track_wheels(self):
        """Draw road wheels on the track - WIREFRAME"""
        wheel_radius = 0.15
    ...
    def draw_tank_turret(self, width, height, depth):
        """Draw tank turret with rounded edges - WIREFRAME"""
        w, h, d = width/2, height/2, depth/2
    ...
    def draw_tank_barrel(self):
        """Draw detailed tank cannon barrel - WIREFRAME"""
        barrel_length = 1.4
        barrel_radius = 0.08
        segments = 8
        
        # Main barrel cylinder - wireframe rings
        ...
        glBegin(GL_LINES)
        for i in range(4):  # 4 longitudinal lines
            angle = (i / 4) * 2 * math.pi
            x = barrel_radius * math.cos(angle)
            y = barrel_radius * math.sin(angle)
            glVertex3f(x, y, 0)
        ...
        # Muzzle brake (additional detail) - wireframe
        glPushMatrix()
    ...
    def draw_tank_details(self, base_color, dark_color):
        """Add extra details like antenna, hatches, etc. - WIREFRAME"""
        # Commander hatch
        glPushMatrix()
...
class Mountain(GameObject):
    def __init__(self, position, color=WHITE):
        super().__init__(position, color)
        self.radius = 1.2  # Reduced from 2.0 for tighter collision
    
    def render(self):
        glPushMatrix()
        glTranslatef(self.position.x, self.position.y, self.position.z)
        
        # Set white/light gray color for wireframe mountains
...
class LifeSphere(GameObject):
    ...
    def render(self):
        ...
        # Set glowing yellow color for wireframe
...
class Laser(GameObject):
    ...
    def render(self):
        ...
        # Set glowing laser color - WIREFRAME
        ...
        # Draw wireframe laser beam as a stretched cube
...
class Game:
    ...
    def init_game_objects(self):
        # Player is now just a position and rotation (no visible tank)
        self.player_position = Vector3(0, 0, 0)
        self.player_rotation_y = 0  # Horizontal rotation
        self.player_speed = 5.0
        self.player_rotation_speed = 90.0
        self.player_radius = 1.0  # For collision detection
        self.last_shot_time = 0
    ...
    def handle_input(self, dt):
        ...
        if keys[pygame.K_SPACE]:
            ...
            if current_time - self.last_shot_time >= self.shot_cooldown:
                ...
                # Calculate shooting direction based on camera rotation (aiming downward)
                direction = Vector3(
                    math.sin(math.radians(self.player_rotation_y)),
                    -0.1,  # Slight downward angle to hit tanks on ground
                    math.cos(math.radians(self.player_rotation_y))
                )
                # Normalize the direction
                direction = direction.normalize()
                
                # Start laser from camera position (slightly forward and at eye height)
                laser_start = Vector3(
                    self.player_position.x + direction.x * 1,
                    self.player_position.y + 1.5,  # Eye height
    ...
    def update_camera(self):
        # First-person camera - camera IS the player
        camera_height = 1.8  # Eye level height
    ...
    def update_game(self, dt):
        ...
        # Update all game objects (no player tank to update)
    ...
    def check_collisions(self):
        ...
        for mountain in self.mountains:
            if self.distance(self.player_position, mountain.position) < (self.player_radius + mountain.radius):
                ...
                self.screen_shake_time = 0.5  # Shake for 0.5 seconds
                self.screen_shake_intensity = 20  # Shake intensity in pixels
                self.red_flash_time = 0.3  # Flash for 0.3 seconds
                self.red_flash_intensity = 0.6  # Flash intensity (0-1)
    ...
    def render_game(self):
        ...
        # Render all objects (no player tank)
        ...
        glDisable(GL_FOG)
        glEnable(GL_LIGHTING)  # Re-enable lighting for HUD (will be disabled again in HUD)
        self.render_hud()
    ...
    def render_hud(self):
        ...
        if self.red_flash_time > 0:
            glEnable(GL_BLEND)
            glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA)
            fade = self.red_flash_time / 0.3  # Fade out over time
    ...
    def draw_sniper_scope(self):
        ...
        # Outer circle (main scope view)
        outer_radius = 150
        glColor3f(0.0, 1.0, 0.0)  # Green scope color
        glLineWidth(2.0)
        ...
        # Draw crosshair lines (extending to edge of outer circle)
        glLineWidth(1.5)
        glBegin(GL_LINES)
        ...
        # Draw diagonal lines for mil-dot style
        glLineWidth(1.0)
        glBegin(GL_LINES)
        # Top-left to center
        ...
        # Add small dots along crosshairs (mil-dots)
        dot_radius = 2
        dot_spacing = 25
